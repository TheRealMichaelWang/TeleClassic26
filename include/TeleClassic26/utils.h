...
#define TC_LOG_SESSION(log_macro, session, FORMAT_STR, ...) log_macro(FORMAT_STR "(session: %*.s@%s)", ##__VA_ARGS__, TC_PROTOCOL_MAX_STR_LEN, session->username, session->authenticated_service->hostname)
