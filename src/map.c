...
static void tc_map_cache_evict(tc_map_cache_t* cache) {
    ...
    for(pint i = 0; i < max_iters; i++) {
        ...
        if (!current_entry->is_loaded) { current_entry = current_entry->prev; continue; }
...
static void tc_map_cache_open_from_entry(tc_map_cache_t* cache, tc_map_cache_entry_t* entry, tc_thread_pool_t* thread_pool, tc_task_backlog_entry_t schedule_info) {
    ...
    tc_task_backlog_invoke_handler(&schedule_info, thread_pool, &entry->map, TRUE);
}
...
void tc_map_cache_schedule_open(tc_map_cache_t* cache, const pchar* name, tc_thread_pool_t* thread_pool, tc_task_backlog_entry_t schedule_info) {
    ...
    if (list_entry) {
        tc_map_cache_open_from_entry(cache, list_entry, thread_pool, schedule_info);
        ...
        load_task_args->schedule_info = schedule_info;
